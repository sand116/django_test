<template>
  <v-container>
    ...
    <v-data-table :headers="headers" :items="posts" sort-by="names" class="elevation-1" :items-per-page="5"
      @click:row="serverPage">
      <template v-slot:top>
        <v-toolbar flat color="white">
          <v-toolbar-title>Post List</v-toolbar-title>
          <span v-if="tagname" class="body-1 font-italic ml-3">(with {{ tagname }} tageed)</span>
          <v-divider class="mx-4" inset vertical></v-divider>
          <v-spacer></v-spacer>
          <!-- dialog 밖에 click버튼이 있는 경우 stop으로 전파를 막아야 함 -->
          <v-btn color="primary" dark class="mb-2" @click.stop="dialogOpen(item, 'create')">
            New Post
          </v-btn>
        </v-toolbar>
      </template>
      ...
      <v-card>
        ...
        <v-card-text>
          <v-form id="post-form" ref="postForm">
            ...
            <div class="mx-3">
              <div class="mx-1">
                <v-text-field name="owner" label="OWNER" readonly v-model="editedItem.owner"></v-text-field>
              </div>
            </div>
            <div class="mx-3">
              <div class="mx-1">
